...
List<Car> carList= new ArrayList<Car>();
	...
	public List<Car> search(String keyword,List<Car> result){
...
//				}
//			}
			result.addAll(carList.stream().filter(car->car.getModel().toLowerCase().contains(keyword.toLowerCase())
										||car.getMake().toLowerCase().contains(keyword.toLowerCase())).collect(Collectors.toList()));
		}
		return result;
	}
}
